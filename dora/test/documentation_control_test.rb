...
template = YAML.load_file(File.join(ROOT, "templates/capability-documentation-adoption-plan.yaml"))
abort "adoption template grants verification authority" unless template.dig("tasks", 0, "evidence_boundary").join(" ").include?("not new delivery verification evidence") || template.dig("scope_control", "non_duplication_rule").include?("not new delivery verification evidence")
